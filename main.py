...
from schemas.schemas import UpdateUserInfoItem, Booking

# 创建表
# models.models.Base.metadata.drop_all(bind=engine)
...
async def update_user_info(
    updateUserInfoItem: UpdateUserInfoItem, db: Session = Depends(get_db)
):
    db_user = (
        db.query(models.models.User).filter_by(token=updateUserInfoItem.token).first()
    )
    if db_user is None:
        return {"code": 201, "message": "您已退出登录"}

    db_user.name = updateUserInfoItem.name
    db_user.company = updateUserInfoItem.company
    try:
        db.commit()
        db.refresh(db_user)
...
async def get_meeting_room_order_by_date(
    ...
    # 查询用户信息
    db_user = db.query(models.models.User).filter_by(token=token).first()
    # 判断用户是否存在
    if db_user is None:
        return {"code": 201, "message": "您已退出登录"}
    # 查询status为1的会议室信息
    db_meeting_room = (
        db.query(models.models.MeetingRoom)
    ...
    db_meeting_room_order_list = (
        db.query(models.models.MeetingRoomOrder)
...
async def get_meeting_room_order_list(
    ...
    if db_user is None:
        return {"code": 201, "message": "您已退出登录"}
    # 获取预约总数
    booking_count = db.query(models.models.Booking).count()
    # 获取预约列表
    booking_list = (
        db.query(models.models.Booking)
...
async def toBooking(booking: Booking = Body(...), db: Session = Depends(get_db)):
    # 验证token
    db_user = db.query(models.models.User).filter_by(token=booking.token).first()
    # 判断用户是否存在
    if db_user is None:
        return {"code": 201, "message": "您已退出登录"}
    # 其中还需要一些判断是否能预约的操作
    ...
    if meetingRoom is None:
        return {"code": 201, "message": "会议室不存在"}
    # 判断会议室是否可用
    if meetingRoom.is_delete == 1:
        return {"code": 201, "message": "会议室不可用"}
    print(booking)
    # 生成预约记录
    db_booking = models.models.Booking(
...
async def update_booking_status(
    updateBookStatusItem: UpdateBookingStatusItem, db: Session = Depends(get_db)
):
    # 查询用户信息
    db_user = (
        db.query(models.models.User).filter_by(token=updateBookStatusItem.token).first()
    )
    # 判断用户是否存在
    if db_user is None:
        return {"code": 201, "message": "您已退出登录"}
    ...
    db_booking = (
        db.query(models.models.Booking)
        .filter_by(id=updateBookStatusItem.booking_id)
        .first()
    )
    # 判断预约信息是否存在
    ...
    # 更新预约状态
    db_booking.booking_status = updateBookStatusItem.booking_status
    # 更新审批人id
...
async def get_booking_message_list(
    ...
    if db_user is None:
        return {"code": 201, "message": "您已退出登录"}
    # 获取预约总数
    booking_message_count = db.query(models.models.Booking).count()
    # 获取预约列表
